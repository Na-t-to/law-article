...
(() => {
  const TOPIC = "cyber-supply-chain";
  const ISSUE = "cyber-vendor-selection";
  const SOURCE = "source-ipa-jc-star";
  const ARTICLE = "article-tmi-unitis-jc-star-20260915";
  ...
  const source = {
    ...
    type: "guideline",
    typeLabel: "IPA・IoT製品セキュリティ適合性評価／ラベリング制度",
    authority: "独立行政法人情報処理推進機構（IPA）",
    publishedAt: "2025-03-25",
    url: "https://www.ipa.go.jp/security/jc-star/index.html",
    importance: "高",
    whyImportant: "IoT製品のセキュリティ要件への適合性を共通基準で評価・可視化するJC-STARの公式ハブ。★1の運用開始、★2～4の制度設計・要件、通信機器・ネットワークカメラの★3要件、英国PSTI・シンガポールCLSとの相互承認、適合ラベル取得製品、誤認表示への注意喚起等の現行情報を確認できる。",
    topics: [TOPIC]
  };

  let currentSources = Array.isArray(window.SOURCE_DATA) ? window.SOURCE_DATA : [];
  const hasSourceId = currentSources.some((item) => item && item.id === SOURCE);
  const hasSourceUrl = currentSources.some((item) => normalizeUrl(item && item.url) === normalizeUrl(source.url));
  if (!hasSourceId && !hasSourceUrl) currentSources = currentSources.concat([source]);
  window.SOURCE_DATA = currentSources;

  window.TOPIC_DATA = (Array.isArray(window.TOPIC_DATA) ? window.TOPIC_DATA : []).map((topic) => {
    if (!topic || topic.slug !== TOPIC) return topic;
    const issues = (Array.isArray(topic.issues) ? topic.issues : []).map((issue) => issue && issue.id === ISSUE
      ? { ...issue, sourceIds: addUnique(issue.sourceIds, SOURCE) }
      : issue);
    return {
      ...topic,
      issues,
  ...
  const article = {
    id: ARTICLE,
    title: "JC-STAR制度の概要とセキュリティ業務への影響（対象製品、レベル、時期等）【2026年9月更新】",
    publisher: "TMI総合法律事務所／UNITIS",
    author: "寺門峻佑・田山翔・梶原尚樹",
    publishedAt: "2026-09-15",
    collectedAt: "2026-09-23",
    url: "https://unitis.jp/articles/18754/",
    sourceType: "secondary",
    sourceLabel: "法律事務所・実務解説／JC-STAR・IoT製品セキュリティ",
    status: "adopted",
    summary: "2025年12月公開記事を2026年9月時点の制度運用に合わせて更新したJC-STARの実務解説。幅広いIoT製品を対象とする4段階の評価・ラベリング制度について、★1・★2は自己適合宣言、★3・★4は第三者評価・認証という差、通信機器・ネットワークカメラの★3要件、英国PSTI・シンガポールCLSとの相互承認を整理する。ベンダー側では対象製品の洗い出し、製品類型判定、要件とのGAP分析、対応計画、部門横断体制を示し、調達側では適合ラベルを製品選定・サプライチェーンリスク低減・情報透明性のための共通指標として使う視点を提示する。",
    whyImportant: [
      "制度の概要紹介にとどまらず、IoT製品ベンダーが製品棚卸し→製品類型判定→GAP分析→対応計画へ落とす実装フローを示している",
      "調達・利用企業の側から、JC-STARを製品選定の共通指標として使い、サプライチェーンのセキュリティリスクと評価コストを下げるという使い方を整理している",
      "★1・★2の自己適合宣言と★3・★4の第三者評価の違いを明確にしており、ラベルのレベルだけを見て保証範囲を過大評価するのを避けられる",
      "IPAの誤認表示への注意喚起と景品表示法上の優良誤認リスクにも触れ、認証取得前の営業・広告表示まで法務の確認対象になることが分かる"
    ],
    audience: ["企業法務", "情報セキュリティ", "調達・購買", "IoT製品開発", "広告・コンプライアンス担当"],
    audienceReason: "IoT製品の開発・販売側では認証取得の準備を、調達側では製品セキュリティ評価の標準化を進め、あわせてJC-STAR表示の誤認リスクを管理するため。",
    categories: ["個人情報・AI・情報セキュリティ", "契約・取引"],
    relatedTopics: [TOPIC],
    relatedIssues: [ISSUE],
    primarySourceIds: [SOURCE],
    legacyReformInference: false,
    whatChanged: "整理変更なし／2026年9月更新版の実務解説を追加し、既存の取引先・調達先評価論点をJC-STARという製品単位の公的評価指標、認証レベル差、誤認表示リスクの観点から補強した。"
  };

  let currentArticles = Array.isArray(window.ARTICLE_DATA) ? window.ARTICLE_DATA : [];
  const hasArticleId = currentArticles.some((item) => item && item.id === ARTICLE);
  const hasArticleUrl = currentArticles.some((item) => normalizeUrl(item && item.url) === normalizeUrl(article.url));
  if (!hasArticleId && !hasArticleUrl) currentArticles = currentArticles.concat([article]);
  window.ARTICLE_DATA = currentArticles;
})();