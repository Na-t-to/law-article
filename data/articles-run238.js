...
(() => {
  ...
  const article = {
    id: "article-amt-takeover-guidelines-points-qa-20260903",
    ...
    status: "adopted",
    summary: "2026年7月30日に経済産業省が公表した「企業買収における行動指針」の解釈・ポイント・Q&Aについて、2023年指針を改訂せず趣旨を明確化する位置付けを確認した上で、望ましい買収、真摯な買収提案・真摯な検討、企業価値と買収価格、取締役会の経営判断、複数提案の比較、特別委員会等を整理する。高値だけで望ましい買収とはならない一方、最高価格を選ばない判断には説得的な説明が必要であること、従業員・取引先や経済安全保障等の定性的要素を将来キャッシュフロー又は割引率への影響として評価すること、買収提案とスタンド・アローン等をシナジー・ディスシナジー込みで比較することまで、対象会社取締役会の判断手順へ落としている。",
    whyImportant: [
      "2026年のポイント・Q&Aが2023年指針を改訂するものではなく、誤解や認識の乖離を解消する明確化であることを踏まえ、旧指針と追加された説明を対比できる",
      "高値の提案を機械的に選ぶ／拒絶できないという理解を退けつつ、最高価格と異なる判断では企業価値への具体的効果と株主への説得的な説明が必要という、取締役会の判断・説明責任を具体化している",
      "買収者への質問、定量・定性比較、スタンド・アローンや提携との比較、シナジー／ディスシナジー、実現可能性・許認可・資力まで、買収提案受領後の検討項目を実務フローへ落とせる"
    ],
    audience: ["上場会社の企業法務", "取締役会事務局", "M&A担当", "コーポレートガバナンス担当", "経営企画"],
    audienceReason: "買収提案を受領した際に、価格だけでなく企業価値、実現可能性、競合提案、スタンド・アローン等をどう比較し、どの情報を買収者へ確認し、株主への説明につなげるかを取締役会プロセスとして設計するため。",
    ...
    primarySourceIds: ["source-meti-corporate-takeover-guidelines-clarification-2026", "source-meti-corporate-takeover-guidelines-2023"],
    legacyReformInference: false,
    whatChanged: "参考解説追加／2026年の解釈・ポイント・Q&Aを、価格と企業価値、真摯な検討、定量・定性比較、複数提案、特別委員会まで取締役会実務へ落とす参考解説を追加した。"
  };
  ...
  const topic = (window.TOPIC_DATA || []).find((item) => item && item.slug === "listed-company-takeover-guidelines");
