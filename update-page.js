(() => {
  ...
  if (!update) {
    $("#updatePage").innerHTML = `<div class="missing-topic"><h1>この更新は見つかりませんでした。</h1><a class="back-link" href="topics.html">← テーマ一覧へ戻る</a></div>`;
    return;
  }
  ...
  $("#updatePage").innerHTML = `<section class="update-hero"><div class="detail-meta"><strong>更新日 <time datetime="${escapeHtml(update.publishedAt)}">${escapeHtml(update.publishedAt)}</time></strong><span>${escapeHtml(source?.authority || "LAW / INDEX")}</span><span>${escapeHtml(update.typeLabel)}</span><span>重要度 ${escapeHtml(update.importance)}</span></div><div class="update-tags">${update.tags.map((tag) => `<span>${escapeHtml(tag)}</span>`).join("")}</div><h1>${escapeHtml(update.headline)}</h1><p class="detail-summary">${escapeHtml(update.summary)}</p></section><div class="detail-main update-main"><section class="detail-section"><h2>要点</h2><ul class="key-point-list">${update.keyPoints.map((point) => `<li>${escapeHtml(point)}</li>`).join("")}</ul></section><section class="detail-section"><h2>影響した論点</h2><p class="section-intro">${escapeHtml(update.whatChanged)}</p><div class="affected-list">${affectedIssues}</div></section><section class="detail-section"><h2>更新前 / 更新後</h2><div class="before-after"><div><strong>更新前</strong><p>${escapeHtml(update.before)}</p></div><div><strong>更新後</strong><p>${escapeHtml(update.after)}</p></div></div></section><section class="detail-section"><h2>一次資料</h2>${source ? `<a class="primary-source" href="${escapeHtml(source.url)}" target="_blank" rel="noopener noreferrer"><div><strong>${escapeHtml(source.title)}</strong><span>${escapeHtml(source.authority)} / ${escapeHtml(source.typeLabel)}</span><small>資料公開日 ${escapeHtml(source.publishedAt)}</small></div><p>${escapeHtml(source.whyImportant)}</p><b>元資料を開く ↗</b></a>` : `<p class="section-intro">紐づく一次資料は登録されていません。</p>`}</section><div class="update-bottom-actions"><a class="back-link" href="topics.html">← テーマ一覧へ戻る</a>${primaryTopic ? `<a class="back-link" href="topics/${escapeHtml(primaryTopic.slug)}.html">${escapeHtml(primaryTopic.title)}を見る →</a>` : ""}</div></div>`;
})();
