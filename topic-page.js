(() => {
  ...
  $("#topicPage").innerHTML = `<section class="detail-hero"><div class="detail-meta"><strong>最終確認 <time datetime="${escapeHtml(topic.lastVerified || topic.lastUpdated)}">${escapeHtml(topic.lastVerified || topic.lastUpdated)}</time></strong><span>${topic.categories.map(escapeHtml).join(" / ")}</span><span>${topic.issues.length}論点</span><span>${topic.sourceIds.length}主要資料</span></div><h1>${escapeHtml(topic.title)}</h1><p class="detail-summary">${escapeHtml(topic.summary)}</p></section><div class="detail-main"><section class="detail-section"><h2>テーマの要点</h2>${renderCurrent()}</section><section class="detail-section"><h2>論点</h2>${renderIssues()}</section><section class="detail-section"><h2>関連する記事・資料</h2>${renderArticles()}</section><section class="detail-section"><h2>主要な一次資料</h2>${renderSources()}</section><section class="detail-section"><h2>更新履歴</h2>${renderHistory()}</section><a class="back-link" href="../index.html#topics">← テーマ一覧へ戻る</a></div>`;
})();
