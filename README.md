...
`node scripts/validate-data.mjs` は日付の逆転、status の分布、全論点が `authoritative` のテーマ、`interpreted` / `pending` の views 未登録を監査します。分布の比率を機械的な正解にはせず、見解対立は根拠資料が確認できた場合だけ登録します。
